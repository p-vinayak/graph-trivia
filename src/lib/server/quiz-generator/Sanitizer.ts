...
export function sanitizeEquations(equationVariances: string[]) {
	const sanitizedEquations = equationVariances.map((equation) => sanitizeEquation(equation));
	return sanitizedEquations;
}

export function sanitizeEquation(equation: string) {
